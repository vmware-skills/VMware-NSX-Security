...
- **Read-only mode**: under `VMWARE_READ_ONLY=true` the 11 tools marked `[WRITE]` — every L3 entry above, `run_traceflow` included — never appear in `list_tools()`. Classification follows each tool's `[READ]`/`[WRITE]` docstring marker; see README.
