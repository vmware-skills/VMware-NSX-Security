...
class TargetConfig:
    ...
    def get_username(self, target_name: str) -> str:
        ...
        Convention: VMWARE_NSX_SECURITY_<TARGET>_USERNAME
        where <TARGET> is upper-cased with hyphens replaced by underscores.
        ...
        """
        env_key = f"VMWARE_NSX_SECURITY_{target_name.upper().replace('-', '_')}_USERNAME"
...
def resolve_config_path(config_path: Path | None = None) -> Path:
    ...
    again in :func:`vmware_nsx_security.doctor.run_doctor`, and the doctor's
    copy had no env-var clause — so with ``VMWARE_NSX_SECURITY_CONFIG`` set it
