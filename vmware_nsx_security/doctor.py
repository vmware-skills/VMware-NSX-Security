...
def _config_read_only() -> bool | None:
    ...
    Deliberately mirrors ``vmware_nsx_security.mcp_server.server._config_read_only`` instead of
    importing it: importing that module applies the gate as a side effect. The
    path comes from ``VMWARE_NSX_SECURITY_CONFIG`` rather than this command's
...
def _check_read_only() -> tuple[bool, str]:
    ...
    status = read_only_status("vmware-nsx-security", _config_read_only())
    if not status.recognised:
        return True, (
            f"{status.source}={status.raw!r} is not a recognised value. It resolves "
            f"to ON (fail-closed), so all 11 write tools are withheld — probably not "
            f"what was intended. Use true or false."
        )
    if status.enabled:
        return True, (
            f"ON (from {status.source}) — 11 write tools withheld, 10 read tools "
            f"exposed. Clear that switch and restart the server to expose them."
        )
    return True, f"off (from {status.source}) — all 21 tools are exposed"
...
def run_doctor(
    ...
    if path.exists():
        ...
        checks.append(
            (
                "Config file",
                False,
                f"Not found: {path}. Run 'vmware-nsx-security init' to create it "
                f"(or copy config.example.yaml to {CONFIG_FILE} by hand).",
            )
        )
    ...
    try:
        import vmware_nsx_security.mcp_server.server  # noqa: F401

        checks.append(("MCP server import", True, "vmware_nsx_security.mcp_server.server importable"))
