...
# Target "nsx-prod" -> VMWARE_NSX_SECURITY_NSX_PROD_{USERNAME,PASSWORD}
_USER_KEY = "VMWARE_NSX_SECURITY_NSX_PROD_USERNAME"
_PW_KEY = "VMWARE_NSX_SECURITY_NSX_PROD_PASSWORD"
...
def test_hyphenated_target_maps_to_underscored_key(monkeypatch):
    ...
    targets' variables."""
    monkeypatch.setenv("VMWARE_NSX_SECURITY_NSX_LAB_USERNAME", "lab-admin")
...
def test_connection_manager_authenticates_with_env_username(monkeypatch):
    ...
    monkeypatch.setattr("vmware_nsx_security.connection.NsxClient", _fake_client)
    monkeypatch.setenv(_USER_KEY, "vault-admin")
