...
Read-only tools are unaffected under both. An operator who only audits DFW
rules needs no config change whatsoever, now or later.
"""
...
def _server(tmp_path, monkeypatch, rules: str | None):
    ...
    monkeypatch.setenv("VMWARE_NSX_SECURITY_CONFIG", str(_write_config(tmp_path)))
...
def _tag_kwargs(target: str) -> dict:
    return {"vm_id": "vm-1", "tag_scope": "env", "tag_value": "web", "target": target}
...
def test_write_against_undeclared_target_warns_but_runs(baseline, caplog) -> None:
    ...
    with caplog.at_level("WARNING"), patch.object(
        tags, "_get_connection", return_value=MagicMock()
    ), patch("vmware_nsx_security.ops.tags.apply_vm_tag", return_value={"tagged": True}) as ops_tag:
        result = baseline.apply_vm_tag(**_tag_kwargs(UNDECLARED))

    ops_tag.assert_called_once()
    # The decorator adds _undo_id to undoable writes, so compare the payload.
...
def test_write_against_undeclared_target_is_denied_when_enforcing(enforcing) -> None:
    ...
    with patch.object(tags, "_get_connection", return_value=MagicMock()):
        with pytest.raises(PolicyDenied) as denied:
            enforcing.apply_vm_tag(**_tag_kwargs(UNDECLARED))
...
def test_write_against_declared_target_succeeds(request, fixture) -> None:
    ...
    with patch.object(tags, "_get_connection", return_value=MagicMock()), patch(
        "vmware_nsx_security.ops.tags.apply_vm_tag", return_value={"tagged": True}
    ) as ops_tag:
        result = server.apply_vm_tag(**_tag_kwargs(DECLARED))
...
def test_reads_work_whether_or_not_the_target_declares(request, fixture, target) -> None:
    ...
    envelope = paginated(found, limit=50, total=len(found))

    with patch.object(groups, "_get_connection", return_value=MagicMock()), patch(
        "vmware_nsx_security.ops.security_group.list_groups", return_value=envelope
    ):
        assert server.list_groups(target=target)["items"] == found
