...
reproduced here: with ``VMWARE_NSX_SECURITY_CONFIG`` set,
``vmware-nsx-security doctor`` reported "Config file PASS" and "Config parse
PASS — 1 target(s) configured" against ``~/.vmware-nsx-security/config.yaml``,
while ``load_config()`` — the call every tool makes — raised
``FileNotFoundError`` on the path in the variable.

``load_config`` resolves ``config_path or $VMWARE_NSX_SECURITY_CONFIG or
...
from vmware_nsx_security import config as cfg
from vmware_nsx_security import doctor as doc
...
def sandbox(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("VMWARE_NSX_SECURITY_CONFIG", raising=False)
...
def test_the_env_var_decides_which_file_is_resolved(sandbox, tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("VMWARE_NSX_SECURITY_CONFIG", str(elsewhere))
...
def test_an_explicit_path_still_beats_the_env_var(sandbox, tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("VMWARE_NSX_SECURITY_CONFIG", str(tmp_path / "ignored.yaml"))
...
def test_doctor_fails_when_the_env_var_points_at_a_missing_file(
    ...
    monkeypatch.setenv("VMWARE_NSX_SECURITY_CONFIG", str(missing))
    ...
    assert "1target(s)configured" not in out, (
        ...
        "call raises FileNotFoundError on the path in $VMWARE_NSX_SECURITY_CONFIG"
    )
...
def test_the_mcp_server_does_not_keep_its_own_copy_of_the_precedence():
    ...
    from vmware_nsx_security.mcp_server import _shared as server
